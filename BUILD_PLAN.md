...
| 11 - Journey map rework (bottom-to-top, tap-to-advance, replay) | ✅ Done — bottom-to-top was already in place; added tap-to-replay for any unlocked stage and swapped the auto-advance-after-solve for return-to-map-then-tap-to-play (see the 2026-09-18 ad hoc entry below for exact wiring) |
...
## Ad hoc — Journey/Badge polish round 2 (2026-09-18)

- **Journey**: every unlocked stage (not just the current one) is now tappable to replay, implementing phase 11's replay item; solving a level returns to the journey map instead of auto-loading the next puzzle, implementing phase 11's tap-to-advance item (`App.tsx` now tracks `activeLevel` separately from `furthestLevel` so a replay never disturbs progress). Also: fixed the current-stage arrow pointing the wrong way (was `caret-up` sitting above the dot, now `caret-down`), bigger zone-name labels shown near both the top and bottom of each zone band, a subtle deterministic starfield scatter per zone band, and a `__DEV__`-only "Test: Solve" button mirroring Badge Challenge's.
- **Badges**: fixed a real bug where `BadgeGame` recomputed `canvasSize` from the *linearized* graph's node count (which can exceed the original count — `singleLineify` duplicates nodes at revisited junctions), while the shape had actually been laid out using the smaller original `canvasSize` from the parent's effect — the mismatch could center the camera on the wrong point and leave the badge off-screen at open. Now `canvasSize` is computed once and passed down as a prop instead of recomputed. Also added a bigger solve celebration (a screen-space Skia confetti burst plus a bouncing "badge earned" banner, `ADVANCE_DELAY_MS` extended to give it room to play), and reworked `BadgeCollectionScreen`'s cards: a solved card now gets a distinct styled look (mint-tinted background/border, a ribbon icon, a brighter/thicker/glowing thumbnail) instead of looking identical to an unsolved one bar a text label, and the first time the collection screen is shown right after solving a badge, that card animates from its plain look to the styled one.
- **General**: rope endpoints (in both normal levels and Badge Challenge) now use a single fixed, more saturated accent (`palette.ts`'s `ENDPOINT_ACCENT`) instead of a color derived from the level's own softer palette, so they read as clearly brighter than a regular node on every palette.

## Ad hoc — Journey/Badge polish round 3 (2026-09-18)

- **Navigation overhaul**: every screen but Settings dropped its "Back" text button for the same home-icon + settings-gear icon pair PuzzleScreen already had — Journey, Badge Challenge's collection screen, and a badge's own puzzle screen. Home always goes straight to the main menu now (no more remembering "where this screen was opened from"), which let `App.tsx` drop its `screenBeforeJourney` ref entirely; Settings keeps its own "return to wherever you were" behavior since that's a different, intentional case.
- **Journey now shows the whole current zone** (not just a short lookahead) with its unsolved levels greyed out as before, **plus the entire next zone** rendered as a flat neutral-grey band with its name replaced by "???" until actually reached. Finishing a zone's last level plays a one-time full-screen reveal (the new zone's own color + name) on the Journey map — tracked via `App.tsx`'s new `justUnlockedZone` flag, consumed once, and only fires on a genuine first-time crossing (checked against `furthestLevel`) so replaying an old level near a zone boundary doesn't retrigger it.
- **Dev tooling audit**: every dev-only control is now behind `__DEV__` and commented as such for easy stripping later — this caught PuzzleScreen's level-jump picker ("Lvl N" button + modal), which was shipping unguarded in every build. Added matching dev-only "Reset" buttons to the Journey and Badge Challenge screens (resetting just that screen's own progress, separate from Settings' full reset).
- **Fixed** BadgeCollectionScreen's "Earned" status text clipping past the card's own border (a fixed square card didn't have enough height for thumbnail + title + hint + status on typical phone widths — cards are taller now) and a card that had already animated once staying visually "earned" forever even after a dev reset flipped it back to unsolved.
